...
# Directories
indir <- "data/extracted_data/raw_round1/timeseries"
...
results <- purrr::map_df(files2eval, function(x){
  ...
  sdata <- sdata_orig %>% 
    ...
    select(., -any_of(c("figure", "note", "week", "date", "hour", "time", "day_orig", "toxicity_sd", "data_source"))) %>%  
    # Add id
...
# Export data
xlsx::write.xlsx(x=results1, file=file.path(outdir, "fitted_model_results_round1.xlsx"))
