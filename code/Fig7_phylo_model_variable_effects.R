...
g1 <- ggplot(coefs_fe, aes(x=beta_med, 
  ...
  geom_point() +
  # Labels
  labs(x="Change in log depuration rate\n(per unit change)", y="", tag="A", title="Fixed effects coefficients") +
  tidytext::scale_y_reordered() +
  # Legend
...
g2 <- ggplot(re, aes(y=tidytext::reorder_within(group, est, type), 
  ...
  geom_point() +
  # Labels
  labs(x="Effect on log depuration rate\n", y="", tag="B", title="Random effects coefficients") +
  tidytext::scale_y_reordered() +
  # Legend
...
# Export
ggsave(g, filename=file.path(plotdir, "Fig7_phylo_model_effects.png"), 
